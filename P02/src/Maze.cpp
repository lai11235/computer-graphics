...
//======================================================================
{
	...
	viewer_dir = 0.0;
	viewer_fov = 45.0;
	tan_fov = tanf(viewer_fov / 360.0f * 3.14159265358f);
...
{
	viewer_fov = f;
	tan_fov = tanf(f / 360.0f * 3.14159265358f);
}
...
Draw_Wall(const float start[2], const float end[2], const float color[3])
{
	...
	glBegin(GL_POLYGON);
	glColor3fv(color);
	MyGlu::Vertex3(Vector(edge0[X],  1.0f, edge0[Z], 1.0f));
	MyGlu::Vertex3(Vector(edge1[X],  1.0f, edge1[Z], 1.0f));
	MyGlu::Vertex3(Vector(edge1[X], -1.0f, edge1[Z], 1.0f));
	MyGlu::Vertex3(Vector(edge0[X], -1.0f, edge0[Z], 1.0f));
	glEnd();
}
...
bool Maze::
Is_Clip(Direction direction[2], int edge_index, Direction view_direction)
{
	if (direction[0] == Direction::LEFT && direction[1] == Direction::RIGHT)
		return true;
	if (direction[0] == Direction::RIGHT && direction[1] == Direction::LEFT)
		return true;
	if (view_direction == Direction::LEFT)
	{
		if (direction[0] == Direction::RIGHT && direction[1] == Direction::RIGHT)
			return true;

		bool is_start_in = (direction[0] == Direction::INCLUDING);
		bool is_end_in = (direction[1] == Direction::INCLUDING);

		if (is_start_in && direction[1] == Direction::RIGHT)
			return true;
		if (is_end_in && direction[0] == Direction::RIGHT)
			return true;
	}

	else if (view_direction == Direction::RIGHT)
	{
		if (direction[0] == Direction::LEFT && direction[1] == Direction::LEFT)
			return true;

		bool is_start_in = (direction[0] == Direction::INCLUDING);
		bool is_end_in = (direction[1] == Direction::INCLUDING);

		if (is_start_in && direction[1] == Direction::LEFT)
			return true;				 
		if (is_end_in && direction[0] == Direction::LEFT)
			return true;
	}
...
Draw_Cell(Cell* cell, LineSeg left, LineSeg right)
{
	...
	for (int i = 0; i < 4; i++)
	{
		LineSeg line(cell->edges[i]);
		float left_rate, left_rate_reverse;
		float right_rate, right_rate_reverse;

		Direction left_direction[2] = { MyGlu::PointDirection(left , line.start), MyGlu::PointDirection(left , line.end)};
		Direction right_direction[2] = { MyGlu::PointDirection(right, line.start), MyGlu::PointDirection(right, line.end)};
			
		if (!Is_Clip(left_direction, i, Direction::LEFT) || !Is_Clip(right_direction, i, Direction::RIGHT))
			continue;

		left_rate          = line.Cross_Param(left);
		right_rate         = line.Cross_Param(right);
		left_rate_reverse  = left.Cross_Param(line);
		right_rate_reverse = right.Cross_Param(line);
		...
		if (cell->edges[i]->opaque)
		{
			...
			case 0:
				...
				if (left_rate < 0 || left_rate > 1)    left_rate = 1;
				if (right_rate > 1 || right_rate < 0)  right_rate = 0;

				temp = line.end[1];
				if(left_rate_reverse > 0)
					line.end[1] = (line.end[1] - line.start[1]) * left_rate + line.start[1];
				if(right_rate_reverse > 0)
					line.start[1] = (temp - line.start[1]) * right_rate + line.start[1];

				break;

			case 1:

				if (left_rate < 0 || left_rate > 1)    left_rate = 0;
				if (right_rate > 1 || right_rate < 0)  right_rate = 1;

				temp = line.start[0];
				if (left_rate_reverse > 0)
					line.start[0] = (line.end[0] - line.start[0]) * left_rate + line.start[0];
				if (right_rate_reverse > 0)
					line.end[0] = (line.end[0] - temp) * right_rate + temp;
			...
			case 2:
				...
				temp = line.start[1];
				if (left_rate_reverse > 0)
					line.start[1] = (line.end[1] - line.start[1]) * left_rate + line.start[1];
				if (right_rate_reverse > 0)
					line.end[1] = (line.end[1] - temp) * right_rate + temp;
			...
			case 3:
				...
				temp = line.end[0];
				if (left_rate_reverse > 0)
					line.end[0] = (line.end[0] - line.start[0]) * left_rate + line.start[0];
				if (right_rate_reverse > 0)
					line.start[0] = (temp - line.start[0]) * right_rate + line.start[0];

				break;

			}
			
			Draw_Wall(line.start, line.end, cell->edges[i]->color);
		}
		else
		{
			if (!(cell->edges[i]->Neighbor(cell)->bFootPrint))
			{
				...
				if (i == 0 || i == 3)
				{
					if (left_rate < 0 || left_rate > 1)
					{
						newL.end[0] = cell->edges[i]->endpoints[Edge::END]->posn[Vertex::X];
						newL.end[1] = cell->edges[i]->endpoints[Edge::END]->posn[Vertex::Y];
					}

					if (right_rate < 0 || right_rate > 1)
					{
						newR.end[0] = cell->edges[i]->endpoints[Edge::START]->posn[Vertex::X];
						newR.end[1] = cell->edges[i]->endpoints[Edge::START]->posn[Vertex::Y];
					}
				}
				...
				else
				{
					if (left_rate < 0 || left_rate > 1)
					{
						newL.end[0] = cell->edges[i]->endpoints[Edge::START]->posn[Vertex::X];
						newL.end[1] = cell->edges[i]->endpoints[Edge::START]->posn[Vertex::Y];
					}

					if (right_rate < 0 || right_rate > 1)
					{
						newR.end[0] = cell->edges[i]->endpoints[Edge::END]->posn[Vertex::X];
						newR.end[1] = cell->edges[i]->endpoints[Edge::END]->posn[Vertex::Y];
					}
				}
...
//======================================================================
{
	...
	for (int i = 0; i < num_cells; i++)cells[i]->bFootPrint = false;
	
	
	float dir = MyGlu::AngleNormalization(viewer_dir);
	
	LineSeg left(0, 0, 0, 0);
	LineSeg right(0, 0, 0, 0);
	float leftDir = dir + viewer_fov / 2;
	float rightDir = dir - viewer_fov / 2;


	if (cosf(Maze::To_Radians(leftDir)) >= 0)
		left = LineSeg(viewer_posn[Maze::X],     viewer_posn[Maze::Y]
				     , viewer_posn[Maze::X] + 1, viewer_posn[Maze::Y] + tanf(Maze::To_Radians(leftDir)));
	else
		left = LineSeg(viewer_posn[Maze::X],     viewer_posn[Maze::Y]
					 , viewer_posn[Maze::X] - 1, viewer_posn[Maze::Y] - tanf(Maze::To_Radians(leftDir)));

	if (cosf(Maze::To_Radians(rightDir)) >= 0)
		right = LineSeg(viewer_posn[Maze::X],     viewer_posn[Maze::Y]
					  , viewer_posn[Maze::X] + 1, viewer_posn[Maze::Y] + tanf(Maze::To_Radians(rightDir)));
	else
		right = LineSeg(viewer_posn[Maze::X],     viewer_posn[Maze::Y]
			          , viewer_posn[Maze::X] - 1, viewer_posn[Maze::Y] - tanf(Maze::To_Radians(rightDir)));
	
	Draw_Cell(view_cell, left, right);
}
